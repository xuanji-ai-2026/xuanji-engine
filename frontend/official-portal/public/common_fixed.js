...
async function loadNavigationAndFooter() {
    try {
        // 加载导航栏
        const navResponse = await fetch('/public/navigation.html');
        if (navResponse.ok) {
        ...
        console.error('加载导航栏或页脚失败:', error);
    }
}
...
function generateBreadcrumb() {
    ...
    if (parts.length > 0) {
        ...
        if (parts.length > 1) {
            ...
            if (pageName) {
                breadcrumb += ` / <span class="text-gray-300">${pageName.replace(/\b\w/g, l => l.toUpperCase())}</span>`;
            }
        }
    }
